...
class FractionalCrossAttentionModule(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
        ...
        self.conv_fuse90 = nn.Sequential(
            nn.Conv3d(2 * r, r, kernel_size=1),
            nn.ReLU(inplace=True)
        )

        self.norm_pre_attn = norm_layer(dim)
        self.cross_attn_x = CrossAttention3D(dim, num_heads_attn, qkv_bias, attn_drop, drop)
        self.cross_attn_xa = CrossAttention3D(dim, num_heads_attn, qkv_bias, attn_drop, drop)
    ...
    def _stack_real_imag(z: torch.Tensor) -> torch.Tensor:
        ...
        real_imag = real_imag.permute(0, 1, 5, 2, 3, 4)
    ...
    def _compute_reduced_features(self, x):
        ...
        out_log = self.frft90.IFrFT3D(z_log).real
    ...
    def forward_part1(self, x, xa):
        ...
        xa_norm = self.norm1_xa(xa)
        rx = self._compute_reduced_features(x_norm)
        rxa = self._compute_reduced_features(xa_norm)

        tx = rearrange(rx, 'b c d h w -> b (d h w) c')
        txa = rearrange(rxa, 'b c d h w -> b (d h w) c')
        tx = self.norm_pre_attn(tx)
        txa = self.norm_pre_attn(txa)

        ax = self.cross_attn_x(tx, txa)
        axa = self.cross_attn_xa(txa, tx)

        B, _, C = ax.shape
        D, H, W = rx.shape[2:]
        ox = ax.view(B, D, H, W, C)
        oxa = axa.view(B, D, H, W, C)
        return ox, oxa
...
class FractMorph(nn.Module):
    ...
    def __init__(self, patch_size=(4,4,4), in_chans=1, embed_dim=48,
        ...
        dpr = list(torch.linspace(0, drop_path_rate, total_blocks).numpy())
        self.layers = nn.ModuleList()
        cur = 0
        for i, depth in enumerate(depths):
            layer = FractBasicLayer(
                dim=embed_dim * 2**i, depth=depth,
        ...
        self.reverse_patch_embedding = nn.ConvTranspose3d(
            embed_dim * 2, embed_dim // 2, kernel_size=patch_size, stride=patch_size
        )
    ...
    def forward(self, moving, fixed):
        ...
        if moving.dim() == 5:
            moving = rearrange(moving, 'b c d h w -> b d h w c')
            fixed = rearrange(fixed, 'b c d h w -> b d h w c')
        else:
            moving = moving.unsqueeze(1).permute(0,2,3,4,1)
            fixed  = fixed.unsqueeze(1).permute(0,2,3,4,1)

        feats_m, feats_f = [], []
        for layer in self.layers:
            out_m, out_f, moving, fixed = layer(moving, fixed)
            feats_m.append(out_m); feats_f.append(out_f)

        moving = self.norm(moving)
        fixed = self.norm(fixed)

        for idx, layer_up in enumerate(self.up_layers):
            rev = len(self.up_layers) - 1 - idx
            if idx == 0:
                _, _, moving, fixed = layer_up(moving, fixed)
            else:
                skip_m, skip_f = feats_m[rev], feats_f[rev]
                if moving.shape[:4] != skip_m.shape[:4]:
                    moving = rearrange(F.interpolate(
                        rearrange(moving,'b d h w c->b c d h w'),
                        size=skip_m.shape[1:4], mode='trilinear', align_corners=True
                    ), 'b c d h w->b d h w c')
                    fixed = rearrange(F.interpolate(
                        rearrange(fixed,'b d h w c->b c d h w'),
                        size=skip_f.shape[1:4], mode='trilinear', align_corners=True
                    ), 'b c d h w->b d h w c')

                moving = self.concat_back_dim[idx](torch.cat([moving, skip_m], dim=-1))
                fixed  = self.concat_back_dim[idx](torch.cat([fixed,  skip_f], dim=-1))
                _, _, moving, fixed = layer_up(moving, fixed)
...
class Head(nn.Module):
    """Full registration head for FractMorph: FractMorph Transformer → UNet → Spatial Transformers."""
